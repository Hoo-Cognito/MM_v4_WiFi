...
MODEL_MM=283 #Original value: 283, switched to 354 to support wifi and MMP fw on MM
MODEL_MMP=354
DEVICE_ID_WifiHack=354 #Original value: 283 switched to 354 to support wifi and MMP fw on MM
#For battery see     # Check is charging, AXP_Status is for MMP, GPIO59 is for MM
...
check_off_order() {
    if [ -f /tmp/.offOrder ]; then
        ...
        bootScreen "$1" &
        sleep 0.01 #SpeedHack # Allow the bootScreen to be displayed
        shutdown
    fi
}
...
mount_main_ui() {
    ...
    mainui_srcname="MainUI-$DEVICE_ID_WifiHack-$mainui_mode" #Hacked to utilize MMP menu, better would be to create a possibility to enable wifi and OTA in the MM menu or a custom menu for MM_Wifi
...
load_settings() {
    ...
    if [ ! -f /mnt/SDCARD/system.json ]; then
        if [ -f /appconfigs/system.json ]; then
            ...
            cp -f $sysdir/res/miyoo${DEVICE_ID}_system.json /mnt/SDCARD/system.json #MM wifi hack
        fi
    fi
